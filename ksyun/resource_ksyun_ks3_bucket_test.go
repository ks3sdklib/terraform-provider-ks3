...
func testSweepOSSBuckets(region string) error {
	...
	for _, v := range resp.Buckets {
		...
		_, err = client.WithKs3Client(func(ks3Client *ks3.Client) (interface{}, error) {
			return nil, ks3Client.DeleteBucket(name)
		})
		if err != nil {
...
func TestAccAlicloudOssBucketBasic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
				...
				Config: testAccConfig(map[string]interface{}{
					"cors_rule": []map[string]interface{}{
						...
						},
						{
							...
							"allowed_headers": []string{"authorization"},
							"expose_headers":  []string{"x-ks3-test", "x-ks3-test1"},
							"max_age_seconds": "100",
						},
					},
				}),
			...
			{
				Config: testAccConfig(map[string]interface{}{
					"policy": `{\"Statement\":[{\"Action\":[\"ks3:*\"],\"Effect\":\"Allow\",\"Resource\":[\"acs:ks3:*:*:*\"]}],\"Version\":\"1\"}`,
				}),
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(nil),
				),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
						...
						"lifecycle_rule.1.noncurrent_version_transition." + hashcode3 + ".storage_class": string(ks3.StorageIA),
						"lifecycle_rule.1.noncurrent_version_transition." + hashcode4 + ".days":          "5",
						"lifecycle_rule.1.noncurrent_version_transition." + hashcode4 + ".storage_class": string(ks3.StorageArchive),
					}),
				),
			},
		},
