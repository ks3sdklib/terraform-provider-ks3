...
cp -rf $CURRENT_PATH/terraform-provider-ks3 src/github.com/aliyun/
...
pushd src/github.com/aliyun/terraform-provider-ks3
...
cp $changelog $CURRENT_PATH/terraform-provider-ks3
popd

cd $CURRENT_PATH
pushd terraform-provider-ks3
git diff | cat
git add .
