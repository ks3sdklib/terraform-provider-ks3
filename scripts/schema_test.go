...
func TestConsistencyWithDocument(t *testing.T) {
	...
	obj := ksyun.Provider().(*schema.Provider).ResourcesMap[*resourceName].Schema
...
func TestFieldCompatibilityCheck(t *testing.T) {
	...
	res := false
	fileRegex := regexp.MustCompile("ksyun/resource[0-9a-zA-Z_]*.go")
	fileTestRegex := regexp.MustCompile("ksyun/resource[0-9a-zA-Z_]*_test.go")
